...
# fb-block 1 start
from catprophet.models import prophet_stan_models
# fb-block 1 end
...
class CatProphet(object):
    """CatProphet forecaster.
    ...
    daily_seasonality: Fit daily seasonality. Can be 'auto', True, or False.
    ...
    def validate_inputs(self):
        ...
        if self.holidays is not None:
            ...
            for h in self.holidays['holiday'].unique():
                ...
                if h in ['zeros', 'yearly', 'weekly', 'daily', 'yhat', 'seasonal',
                         'trend']:
    ...
    def set_auto_seasonalities(self):
        ...
        if self.daily_seasonality == 'auto':
            dt = self.history['ds'].diff()
            min_dt = dt.iloc[dt.nonzero()[0]].min()
            if ((last - first < pd.Timedelta(days=2)) or
                    (min_dt > pd.Timedelta(days=1))):
                self.daily_seasonality = False
                logger.info('Disabling daily seasonality. Run prophet with '
    ...
    def plot_components(self, fcst, uncertainty=True, plot_cap=True,
                        daily_start=0, weekly_start=0, yearly_start=0):
    ...
    def plot_daily(self, ax=None, uncertainty=True, daily_start=0):
        ...
        ax: Optional matplotlib Axes to plot on. One will be created if this
            is not provided.
        uncertainty: Optional boolean to plot uncertainty intervals.
        daily_start: Optional int specifying the start day of the daily
            seasonality plot. 0 (default) starts the day on Sunday. 1 shifts
            by 1 day to Monday, and so on.
        ...
        days = (pd.date_range(start='2017-01-01', periods=1) +
                pd.Timedelta(days=daily_start))
        df_d = pd.DataFrame({'ds': days, 'cap': 1.})
        df_d = self.setup_dataframe(df_d)
        seas = self.predict_seasonal_components(df_d)
        artists += ax.plot(range(len(days)), seas['daily'], ls='-',
                           c='#0072B2')
        if uncertainty:
            artists += [ax.fill_between(range(len(days)),
                                        seas['daily_lower'], seas['daily_upper'],
                                        color='#0072B2', alpha=0.2)]
        ax.grid(True, which='major', c='gray', ls='-', lw=1, alpha=0.2)
